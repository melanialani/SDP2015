...
class Mahasiswa_model extends CI_Model {
	...
	function getDataMahasiswaByNomorRegistrasi($nomor_registrasi)
	{
		$this->db->select('m.nomor_registrasi_id, m.nrp, m.nama, m.sks, m.semester, i.jurusan, i.kategori, i.harga_usp');
		$this->db->from('mahasiswa m, informasi_kurikulum i');
		$this->db->where('m.nomor_registrasi_id', $nomor_registrasi);
		$this->db->where('i.id = m.informasi_kurikulum_id');
		return $this->db->get()->result();
	}
	...
	{
		$this->db->select('m.nrp, m.nama, i.jurusan, m.jenis_kelamin, m.tempat_lahir, m.tanggal_lahir,
	...
	{
		$this->db->select('c.nomor_registrasi_id, c.informasi_kurikulum_id, c.nama, i.jurusan, i.kategori, c.jenis_kelamin, c.tempat_lahir, c.tanggal_lahir,
				c.kewarganegaraan, c.status_sosial, c.agama, c.alamat, c.provinsi, c.kota, c.kodepos, c.nomor_hp, c.email,
				c.nama_wali, c.alamat_wali, c.provinsi_wali, c.kota_wali, c.nomor_telp_wali, c.pekerjaan_wali, c.status
		');
		$this->db->from('calon_mahasiswa c, informasi_kurikulum i');
		$this->db->where('c.email', $email);
		$this->db->where('i.id = c.informasi_kurikulum_id');
	...
	function getFullDataMahasiswaByNomorRegistrasi($nomor_registrasi)
	{
